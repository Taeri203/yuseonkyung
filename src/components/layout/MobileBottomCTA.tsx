    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-[#D7E4FF] bg-white/96 px-2 pb-2 pt-1.5 shadow-2xl backdrop-blur md:hidden" aria-label="모바일 하단 바로가기">
      <div className="grid grid-cols-5 gap-1">
        {items.map((item) => {
          ...
          return (
            <Link key={item.href + item.label} href={item.href} className="flex flex-col items-center gap-0.5 rounded-2xl bg-[#EAF2FF] px-1 py-1.5 text-[11px] font-black text-[#0B1F66]">
              <Icon className="h-5 w-5 text-[#1434A4]" aria-hidden />
              {item.label}
            </Link>
          );
        })}
