...
export function MobileMenu() {
  ...
  const menuPanel = open ? (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-[#0B1F66]/62 p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-label="모바일 메뉴">
      <div className="w-full max-w-[360px] overflow-hidden rounded-[1.75rem] bg-white shadow-[0_30px_80px_rgba(2,8,23,.34)]">
        <div className="flex items-start justify-between gap-4 bg-[#0B1F66] p-5 text-white">
          <div>
        ...
        <nav className="grid grid-cols-2 gap-2 p-4" aria-label="모바일 주요 메뉴">
          {menuLinks.map((item) => (
            <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="rounded-2xl border border-[#E5E7EB] bg-[#F8FAFC] px-4 py-4 text-center text-sm font-black text-[#0B1F66] transition hover:border-[#B9D2FF] hover:bg-[#EAF2FF]">
              {item.label}
            </Link>
          ))}
        </nav>
